$j(document).ready(function(){
    ...
    $j('#registerNewId').click(function(event){
        event.stopPropagation();
        event.preventDefault();
        $j('#searchByNameBtn').click();
    });

    $j('#registerWithExistingId').click(function(event){
        var nextPage = '/module/patientregistration/workflow/enterPatientDemo.form';
        if(patientIdentifier.length>0){
            nextPage = nextPage + '?patientIdentifier=' + patientIdentifier;
        }
        if(nextTask.length>0){
            nextPage = nextPage + '?nextTask=' + nextTask;
        }
